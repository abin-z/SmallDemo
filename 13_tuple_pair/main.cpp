...
/**
 * C++ 中，std::tuple、std::pair 和 std::tie
 * 是用来处理多种数据类型组合的工具。它们允许你将多个值捆绑在一起并传递或返回。
 *
...
void testTuple02()
{
  ...
  fmt::println("std::tuple_element<0, decltype(tp)>::type).name() = {}",
               typeid(std::tuple_element<0, decltype(tp)>::type).name());
  fmt::println("std::tuple_element<1, decltype(tp)>::type).name() = {}",
               typeid(std::tuple_element<1, decltype(tp)>::type).name());
  fmt::println("std::tuple_element<2, decltype(tp)>::type).name() = {}",
               typeid(std::tuple_element<2, decltype(tp)>::type).name());
  fmt::println("std::tuple_element<3, decltype(tp)>::type).name() = {}",
               typeid(std::tuple_element<3, decltype(tp)>::type).name());
  fmt::println("std::tuple_element<4, decltype(tp)>::type).name() = {}",
               typeid(std::tuple_element<4, decltype(tp)>::type).name());
  fmt::println("std::tuple_element<5, decltype(tp)>::type).name() = {}",
               typeid(std::tuple_element<5, decltype(tp)>::type).name());
}
