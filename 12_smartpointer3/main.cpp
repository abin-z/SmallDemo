 ...
 * 
 * 
 * 循环引用（Cyclic Reference）是指两个或多个 std::shared_ptr 互相持有对方，导致它们的引用计数永远无法降为 0，最终导致内存泄漏。
 * std::shared_ptr 依赖引用计数来管理资源。当 shared_ptr 被拷贝时，引用计数 +1；当 shared_ptr 被销毁时，引用计数 -1。
 * 但如果两个对象互相持有 shared_ptr，即使它们超出了作用域，引用计数仍然不会降到 0，导致资源不会被释放。
 * 
...
struct A
{
  ...
  ~A() { fmt::print("A destroyed\n"); }
};

struct B
{
  std::shared_ptr<A> ptrA;
  ~B() { fmt::print("B destroyed\n"); }
};
...
struct AA
{
  ...
  ~AA() { fmt::print("AA destroyed\n"); }
};

struct BB
{
  std::weak_ptr<AA> ptrA; // 使用 weak_ptr 不影响强引用计数的智能指针,它不增加 shared_ptr 的引用计数，因此不会导致循环引用问题
  ~BB() { fmt::print("BB destroyed\n"); }
};
